...
class RegistrationRequestViewSet(viewsets.ViewSet):
    permission_classes = [permissions.IsAuthenticated]
    parser_classes = [MultiPartParser, FormParser]

    def list(self, request):
        user = request.user
        queryset = RegistrationRequest.objects.filter(project__owner=user)
    ...
    def retrieve(self, request, pk=None):
        ...
        instance = get_object_or_404(
            RegistrationRequest, pk=pk, project__owner=user)
        serializer = RegistrationRequestSerializer(instance)
        return Response(serializer.data, status=status.HTTP_200_OK)
    ...
    def create(self, request):
        ...
        for f in files:
            registration_request.files.create(file=f)
