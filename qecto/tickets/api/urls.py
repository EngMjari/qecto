from django.urls import path
from .views import (
    TicketSessionListCreateView, TicketSessionDetailView,
    TicketSessionCloseView, TicketSessionsByRequestView,
    TicketMessageAttachmentView
)

urlpatterns = [
    path('sessions/', TicketSessionListCreateView.as_view(),
         name='ticket-session-list-create'),
    path('sessions/<uuid:session_id>/',
         TicketSessionDetailView.as_view(), name='ticket-session-detail'),
    path('sessions/<uuid:session_id>/close/',
         TicketSessionCloseView.as_view(), name='ticket-session-close'),
    path('sessions/by-request/<uuid:request_id>/<str:request_type>/',
         TicketSessionsByRequestView.as_view(), name='ticket-sessions-by-request'),
    path('messages/<uuid:ticket_id>/<uuid:message_id>/attachments/',
         TicketMessageAttachmentView.as_view(), name='ticket-message-attachment'),
]
