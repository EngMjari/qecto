...
class ExpertEvaluationRequestAdmin(admin.ModelAdmin):
    ...
    def refer_request(self, request, queryset):
        ...
        self.message_user(
            request,
            f"{total_referred} درخواست ارزیابی کارشناسی با موفقیت به {assigned_admin} ارجاع داده شد.",
            level='success'
        )
    ...
    def refer_request_form(self, request, queryset):
        ...
        return TemplateResponse(
            ...
            {
                'title': 'ارجاع درخواست‌های ارزیابی کارشناسی',
    ...
    def get_actions(self, request):
        ...
        if not request.user.is_superuser and not ExpertEvaluationRequest.objects.filter(assigned_admin=request.user).exists():
