...
def check_and_delete_project_if_unused(project):
    if not project:
        ...
        return
    has_survey = SurveyProject.objects.filter(project=project).exists()
    has_expert = ExpertEvaluationProject.objects.filter(project=project).exists()
    print(f"[ProjectSignal] Checking project {project.id} - {project.title}: has_survey={has_survey}, has_expert={has_expert}")
    if not has_survey and not has_expert:
        print(f"[ProjectSignal] Deleting unused project {project.id} - {project.title}")
        project.delete()
    else:
        print(f"[ProjectSignal] Project {project.id} still has requests, not deleting.")


@receiver(post_delete, sender=SurveyProject)
