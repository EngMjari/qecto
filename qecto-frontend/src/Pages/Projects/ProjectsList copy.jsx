...
function ProjectsList() {
  ...
  const [referralType, setReferralType] = useState(null); // 'project' یا 'request'
  const [referralData, setReferralData] = useState(null); // داده‌های ارجاع (project یا request)
  ...
  const handleReferralSubmit = async (formValues) => {
    ...
    try {
      if (referralType === "project") {
        ...
        alert(`پروژه ${referralData.title} با موفقیت ارجاع شد.`);
        fetchData(page); // رفرش داده‌ها
  ...
  // تابع برای گرفتن content_type_id (فرضاً از API)
  const getContentTypeId = async (modelName) => {
    // TODO: پیاده‌سازی API برای گرفتن content_type_id
    // این یه پیاده‌سازی موقت با مقادیر ثابت است
    const contentTypeMap = {
      surveyrequest: 1,
    ...
    return contentTypeMap[modelName] || 1; // باید با API واقعی جایگزین بشه
  };
